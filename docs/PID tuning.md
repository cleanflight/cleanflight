...
keep itself stable. A craft with low P gain might feel as if it was "drunk". The pilot might feel as if they were constantly one step behind the quad--the pilot is responding to the quad, instead of the other way around.
...
**The I term**. Ideally, the P term would be all that is needed to have a perfect controller. In reality, the P term is always a little bit "behind the curve", because the copter takes some time to respond, and by the time it responds, the situation has changed. The I term tracks the cumulative error accrued by the P term and attempts to compensate for it. 

To go back to the "car" analogy, imagine that a car is driving through the desert, and it means to be driving straight to a city that is directly north of it. Now imagine that the driver becomes inattentive, and the car drifts until it is driving northwest, and it continues to drive that direction for a while. Eventually, the driver notices the compass and turns the car back to the north. At this point, the P-term error is zero because the car is _currently_ facing the correct direction. But in fact, the car is some distance west of its intended track, because of the error that occurred in the past. The I term's job is to remember that this error occurred and correct for it. The P term says, "You are facing the correct direction. Drive straight ahead." The I term says, "Sure, everything is fine _now_, but for the last half hour, you were going the wrong direction. Turn right and drive northeast for a half hour, please."
...
**The D term** attempts to increase system stability by monitoring the rate of change in the error. When the P term is changing rapidly, the D term causes the strength of the correction to be backed off. When the P term is stable, the D term is low, allowing the output of the PID controller to move more freely. The D term can be though of like a friction brake that applies more force as the speed of the vehicle increases. When the vehicle is moving slowly, the brake is disengaged. The faster the vehicle goes, the harder the brake pusehs to slow it back down again. In this analogy, the P term is the vehicle and the D term is the brake.
...
To a small extend, the D term has the effect of reducing oscillation in the P term. If the P term is set just barely high enough that oscillations are starting to occur, adding a small amount of D may tame the oscillations. The more significant effect of the D term is that it tames overshoot and bounce on strong stick inputs. 
...
Because of the D term's propensity to amplify noise, it's common to apply a low-pass filter to it. Prior to Cleanflight version 1.9.1, Harakiri (PID controller 5) was the only PID controller to do this. As a result, it wasn't possible to fully tune the D term on the other PID controllers, because by the time D got large enough to really do its job, it was amplifying noise so much that it needed to be turned back down again. In Cleanflight 1.9.1, software filtering of the D term (and the P term and gyro) was added. This filtering is turned off by default for backwards compatibility, but nearly all users report better flight performance with it turned on.
...
As of Cleanflight 1.9.1, TPA is supported in all PID controllers. Prior to 1.9.1, PID controllers 1 and 2 did not support TPA. The TPA parameters can be set in the Cleanflight GUI or CLI, but they will have no effect on PID controllers 1 and 2.

TPA applies a PID value reduction in relation to full throttle. It is used to apply dampening of PID values as full throttle is reached. Prior to Cleanflight 1.9.1, TPA reduced all three of the P/I/D parameters equally on all PID controllers except Harakiri (PID Controller #5). In fact, it's not desirable to reduce the I term as throttle increases, since this means the copter will have reduce attitude-hold right when you most need to to be locked in. In Cleanflight 1.9.1, the I term was removed from the TPA calculation for all PID controllers. The effect is that TPA reduces the propensity for oscillation as throttle increases, but attitude hold is not compromised.
...
Prior to Cleanflight 1.9.1, Harakiri was the only PID controller that had D term low-pass filtering (via the MAIN_CUT_HZ function). From 1.9.1 forward, Harakiri's MAIN_CUT_HZ function was replaced with a set of global filter functions used by all PID controllers.
