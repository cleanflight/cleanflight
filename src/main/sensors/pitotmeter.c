...
pitot_t pitot;                        // barometer access functions
uint16_t calibratingP = 0;      // baro calibration = get new ground pressure value
...
{
    static uint32_t pitotDeadline = 0;
    static pitotmeterState_e state = PITOTMETER_NEEDS_SAMPLES;

    if ((int32_t)(currentTime - pitotDeadline) < 0)
        return;

    pitotDeadline = currentTime;

    switch (state) {
        case PITOTMETER_NEEDS_SAMPLES:
            pitot.start();
            state = PITOTMETER_NEEDS_CALCULATION;
            pitotDeadline += pitot.delay;
        break;
...
int32_t pitotCalculateAirSpeed(void)
{
	...
	//debug[3] = (int16_t)zeropointvalue;
