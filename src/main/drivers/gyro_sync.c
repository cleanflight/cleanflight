...
void gyroUpdateSampleRate(uint32_t looptime, uint8_t lpf, uint8_t syncGyroToLoop) {
    ...
    if (syncGyroToLoop) {
#if defined(SPRACINGF3) || defined(ALIENWIIF3) || defined(NAZE32PRO) || defined(STM32F3DISCOVERY) || defined (MOTOLAB) || defined(CHEBUZZF3) || defined(EUSTM32F103RC) || defined(PORT103R)
        if (!lpf) {
