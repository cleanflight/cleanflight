...
bool ist8310Detect(mag_t* mag, const ist8310Config_t *ist8310ConfigToUse)
{
    ...
    ack = i2cRead(IST8310_ADDRESS, IST8310_REG_WHOAMI, 1, &sig);
...
void ist8310Init(void)
{
    ...
    // 
    i2cWrite(IST8310_ADDRESS, IST8310_REG_CNTRL1, IST8310_ODR_SINGLE);
    delay(5);
    i2cWrite(IST8310_ADDRESS, IST8310_REG_AVERAGE, IST8310_AVG_16);
    delay(5);
    ist8310Read(magADC);
    delay(5);
    i2cWrite(IST8310_ADDRESS, IST8310_REG_CNTRL1, IST8310_ODR_SINGLE);
...
{
    uint8_t buf[6];
    int8_t LSB2FSV = 3; // 3mG - 14 bit
    bool ack = i2cRead(IST8310_ADDRESS, IST8310_REG_DATA, 6, buf);
    if (!ack) {
        return false;
    }
    ...
    // set compass to measure next time data
    i2cWrite(IST8310_ADDRESS, IST8310_REG_CNTRL1, IST8310_ODR_SINGLE);
    return true;
}
