...
static void stopAllServos(void)
{
	for (int index = 0; index < MAX_SUPPORTED_SERVOS; index++)
		servo[index] = servoProfile()->servoConf[index].middle;
}

static void getReceiverMapping(void)
{
    for (int i = 0; i < MAX_MAPPABLE_RX_INPUTS; i++)
    	tempString[rxConfig()->rcmap[i]] = rcChannelLetters[i];
}
...
static void initialiseItemsOfTheSelectedPage(void)
{
	...
	switch (selectedPage){
        ...
        case MOTORS_PAGE : {
	    	...
	    	structureOfPage.editField[21].data = feature(FEATURE_3D) ? motor3DConfig()->neutral3d : motorAndServoConfig()->mincommand;
	    break;
	    }
	    case SERVOS_PAGE : {
			if (servoCount != 0) {
	...
	switch(valueTable[structureOfPage.editField[3].data].type & VALUE_MODE_MASK) {
		case MODE_DIRECT :
			switch (valueTable[structureOfPage.editField[3].data].type & VALUE_TYPE_MASK) {
				case VAR_UINT8:
					valueInteger = *(uint8_t *)cliVarPtr(&valueTable[structureOfPage.editField[3].data]);
				break;
				case VAR_INT8:
					valueInteger = *(int8_t *)cliVarPtr(&valueTable[structureOfPage.editField[3].data]);
				break;
				case VAR_UINT16:
					valueInteger = *(uint16_t *)cliVarPtr(&valueTable[structureOfPage.editField[3].data]);
				break;
				case VAR_INT16:
					valueInteger = *(int16_t *)cliVarPtr(&valueTable[structureOfPage.editField[3].data]);
				break;
				case VAR_UINT32:
					valueInteger = *(uint32_t *)cliVarPtr(&valueTable[structureOfPage.editField[3].data]);
				break;
				case VAR_FLOAT:
					valueFloat = *(float *)cliVarPtr(&valueTable[structureOfPage.editField[3].data]);
...
{
    uint8_t profileIndex = structureOfPage.editField[3].data;
	structureOfPage.editField[6].data = pidProfile_profile(profileIndex)->pidController;
	switch (structureOfPage.editField[6].data){
		case PID_CONTROLLER_MW23 :
		case PID_CONTROLLER_MWREWRITE :
			for (int i = 0; i < 3; i++){
				structureOfPage.editField[firstItemOnLine(i+4)].data = pidProfile_profile(profileIndex)->P8[i];
				structureOfPage.editField[firstItemOnLine(i+4)+1].data = pidProfile_profile(profileIndex)->I8[i];
				structureOfPage.editField[firstItemOnLine(i+4)+2].data = pidProfile_profile(profileIndex)->D8[i];
			}
		break;
		case PID_CONTROLLER_LUX_FLOAT :
			for (int i = 0; i < 3; i++){
				structureOfPage.editField[firstItemOnLine(i+4)].data = round(pidProfile_profile(profileIndex)->P_f[i] * SCALE_P_LUX);
				structureOfPage.editField[firstItemOnLine(i+4)+1].data = round(pidProfile_profile(profileIndex)->I_f[i] * SCALE_I_LUX);
				structureOfPage.editField[firstItemOnLine(i+4)+2].data = round(pidProfile_profile(profileIndex)->D_f[i] * SCALE_D_LUX);
			}
		break;
	}
}
...
static void updateRateValuesForTheCurrentProfile(void)
{
    ...
    structureOfPage.editField[9].data = controlRateProfiles(profileIndex)->rates[FD_ROLL];
	structureOfPage.editField[10].data = controlRateProfiles(profileIndex)->rates[FD_PITCH];
	structureOfPage.editField[11].data = controlRateProfiles(profileIndex)->rates[FD_YAW];
	structureOfPage.editField[15].data = controlRateProfiles(profileIndex)->dynThrPID;
	structureOfPage.editField[18].data = controlRateProfiles(profileIndex)->tpa_breakpoint;
}
...
static void updateValuesForTheSelectedServo(void)
{
    ...
    structureOfPage.editField[6].data = servoProfile()->servoConf[profileIndex].min;
    structureOfPage.editField[7].data = servoProfile()->servoConf[profileIndex].max;
    structureOfPage.editField[9].data = servoProfile()->servoConf[profileIndex].middle;
    structureOfPage.editField[10].data = (servoProfile()->servoConf[profileIndex].rate > 0);
    structureOfPage.editField[11].data = abs(servoProfile()->servoConf[profileIndex].rate);
    structureOfPage.editField[12].data = servoProfile()->servoConf[profileIndex].angleAtMin;
    structureOfPage.editField[15].data = servoProfile()->servoConf[profileIndex].angleAtMax;

    if (servoProfile()->servoConf[profileIndex].forwardFromChannel == (int8_t)CHANNEL_FORWARDING_DISABLED)
...
static void updateValuesForThisPage(void)
{
	switch (selectedPage){
...
#ifdef TELEMETRY
        	structureOfPage.editField[18].data = hottTelemetryConfig()->hottAlarmSoundInterval;
#else
		...
		case BATTERY_PAGE : {
			...
			structureOfPage.editField[6].data = batteryConfig()->vbatmincellvoltage;
			structureOfPage.editField[9].data = batteryConfig()->vbatmaxcellvoltage;
			structureOfPage.editField[12].data = batteryConfig()->vbatwarningcellvoltage;
			structureOfPage.editField[15].data = batteryConfig()->vbatscale;
		break;
		}
		case CURRENTMETER_PAGE : {
			structureOfPage.editField[3].data = featureConfigured(FEATURE_CURRENT_METER) ;
			structureOfPage.editField[6].data = abs(batteryConfig()->currentMeterScale);
			structureOfPage.editField[7].data = (batteryConfig()->currentMeterScale >= 0);
			structureOfPage.editField[9].data = batteryConfig()->currentMeterOffset;
			structureOfPage.editField[12].data = batteryConfig()->multiwiiCurrentMeterOutput;
			structureOfPage.editField[15].data = batteryConfig()->currentMeterType;
		break;
		}
		case ESCMOTORS_PAGE : {
			...
			structureOfPage.editField[9].data = motorAndServoConfig()->minthrottle;
			structureOfPage.editField[12].data = motorAndServoConfig()->servoCenterPulse;
			structureOfPage.editField[15].data = motorAndServoConfig()->maxthrottle;
	    	structureOfPage.editField[18].data = feature(FEATURE_3D) ? motor3DConfig()->neutral3d : motorAndServoConfig()->mincommand;
		break;
		}
	    case PID_PROFILE_PAGE : {
		...
		case GPS_PAGE : {
			...
			structureOfPage.editField[6].data = gpsConfig()->provider;
			structureOfPage.editField[9].data = gpsData.baudrateIndex; // FIXME this isn't where to get the configurated baudrate from.
			structureOfPage.editField[12].data = gpsConfig()->sbasMode;
			int index = getCurrentProfile();
			structureOfPage.editField[15].data = (compassConfig_profile(index)->mag_declination >= 0);
			structureOfPage.editField[16].data = abs(compassConfig_profile(index)->mag_declination / 100);
			structureOfPage.editField[17].data = abs(compassConfig_profile(index)->mag_declination) % 100;
#endif
		break;
		}
		case RECEIVER_PAGE : {
			int index = getCurrentControlRateProfile();
			structureOfPage.editField[3].data = controlRateProfiles(index)->thrMid8;
			structureOfPage.editField[9].data = controlRateProfiles(index)->thrExpo8;
			structureOfPage.editField[10].data = controlRateProfiles(index)->rcExpo8;
			structureOfPage.editField[11].data = controlRateProfiles(index)->rcYawExpo8;
			structureOfPage.editField[12].data = controlRateProfiles(index)->rcRate8;
			structureOfPage.editField[15].data = rxConfig()->rssi_channel;
		break;
		}
	    case SERVOS_PAGE : {
	...
	switch (selectedItem) {
		case 3 :{
			if ((valueTable[structureOfPage.editField[3].data].type & VALUE_MODE_MASK) == MODE_LOOKUP){
				structureOfPage.editField[9].data = *(uint8_t *)cliVarPtr(&valueTable[structureOfPage.editField[3].data]);
...
static void updatePIDProfileValuesInMasterConfig(void)
{
	...
	pidProfile_profile(profile)->pidController = structureOfPage.editField[6].data;

	switch (structureOfPage.editField[6].data){
		case PID_CONTROLLER_MW23 :
		case PID_CONTROLLER_MWREWRITE :
			for (int i = 0; i < 3; i++){
				pidProfile_profile(profile)->P8[i] = structureOfPage.editField[12 + i*3].data;
				pidProfile_profile(profile)->I8[i] = structureOfPage.editField[13 + i*3].data;
				pidProfile_profile(profile)->D8[i] = structureOfPage.editField[14 + i*3].data;
			}
		break;
		case PID_CONTROLLER_LUX_FLOAT :
			for (int i = 0; i < 3; i++){
				pidProfile_profile(profile)->P_f[i] = (float)structureOfPage.editField[12 + i*3].data / SCALE_P_LUX;
                pidProfile_profile(profile)->I_f[i] = (float)structureOfPage.editField[13 + i*3].data / SCALE_I_LUX;
				pidProfile_profile(profile)->D_f[i] = (float)structureOfPage.editField[14 + i*3].data / SCALE_D_LUX;
			}
		break;
	}
}
...
static void applyNewValuesOnTheCurrentPage(void)
{
    switch (selectedPage){
...
#ifdef TELEMETRY
        	hottTelemetryConfig()->hottAlarmSoundInterval = structureOfPage.editField[18].data;
#endif
		break;
		}
        case BATTERY_PAGE : {
        	updateFeatureSelection(structureOfPage.editField[3].data, FEATURE_VBAT);
			batteryConfig()->vbatmincellvoltage = structureOfPage.editField[6].data;
			batteryConfig()->vbatmaxcellvoltage = structureOfPage.editField[9].data;
			batteryConfig()->vbatwarningcellvoltage = structureOfPage.editField[12].data;
			batteryConfig()->vbatscale = structureOfPage.editField[15].data;
        ...
        case CURRENTMETER_PAGE : {
        	...
        	batteryConfig()->currentMeterScale = (structureOfPage.editField[7].data ? 1 : - 1) * structureOfPage.editField[6].data;
        	batteryConfig()->currentMeterOffset = structureOfPage.editField[9].data;
        	batteryConfig()->multiwiiCurrentMeterOutput = structureOfPage.editField[12].data;
        	batteryConfig()->currentMeterType = structureOfPage.editField[15].data;
		break;
		}
        case ESCMOTORS_PAGE : {
			...
			motorAndServoConfig()->minthrottle = structureOfPage.editField[9].data;
			motorAndServoConfig()->servoCenterPulse = structureOfPage.editField[12].data;
			motorAndServoConfig()->maxthrottle = structureOfPage.editField[15].data;
	    	if(feature(FEATURE_3D))
	    		motor3DConfig()->neutral3d = structureOfPage.editField[18].data;
	    	else
	    		motorAndServoConfig()->mincommand = structureOfPage.editField[18].data;
		break;
		}
	    case PID_PROFILE_PAGE : {
            int index = structureOfPage.editField[3].data;
			if (selectedItem == 3) {
			    // FIXME what is this supposed to do, method does not exist?  - perhaps changeProfile?
				//setCurrentPIDprofileConfig(index);
			    changeProfile(index); // FIXME if we do this the changes to the parameter_group.h that expose the *_profile(index) method are not needed and other code in this file should become simpler.

				updatePIDProfileValues();
			}
			else if (selectedItem == 6){
				pidProfile_profile(index)->pidController = structureOfPage.editField[6].data;
				updatePIDProfileValues();
			}
			else {
	    ...
	    case RATE_PROFILE_PAGE : {
			...
			if (selectedItem == 3) {
				...
				controlRateProfiles(index)->rates[FD_ROLL] = structureOfPage.editField[9].data;
				controlRateProfiles(index)->rates[FD_PITCH] = structureOfPage.editField[10].data;
				controlRateProfiles(index)->rates[FD_YAW] = structureOfPage.editField[11].data;
				controlRateProfiles(index)->dynThrPID = structureOfPage.editField[15].data;
				controlRateProfiles(index)->tpa_breakpoint = structureOfPage.editField[18].data;
			}
		break;
	    }
	    case GPS_PAGE : {
	    	...
	    	gpsConfig()->provider = structureOfPage.editField[6].data;
	    	gpsData.baudrateIndex = structureOfPage.editField[9].data; // FIXME this isn't how to change the baudrate and have it saved.
	    	gpsConfig()->sbasMode = structureOfPage.editField[12].data;
	    	...
	    	compassConfig()->mag_declination = declination;
#endif
	    break;
	    }
	    case RECEIVER_PAGE : {
			if (selectedItem == 18)
				...
				int index = getCurrentControlRateProfile();
				controlRateProfiles(index)->thrMid8 = structureOfPage.editField[3].data;

				controlRateProfiles(index)->thrExpo8 = structureOfPage.editField[9].data;
				controlRateProfiles(index)->rcExpo8 = structureOfPage.editField[10].data;
				controlRateProfiles(index)->rcYawExpo8 = structureOfPage.editField[11].data;

				controlRateProfiles(index)->rcRate8 = structureOfPage.editField[12].data;
				rxConfig()->rssi_channel = structureOfPage.editField[15].data;
			}
		break;
	    }
        case MOTORS_PAGE : {
        	for (int index = 0; index < motorCount; index++) {
        		...
        		if (!structureOfPage.editField[18].data)
            		motor_disarmed[index] = feature(FEATURE_3D) ? motor3DConfig()->neutral3d : motorAndServoConfig()->mincommand;
        		else {
	    ...
	    case SERVOS_PAGE : {
			if (selectedItem == 3)
				...
				int index = structureOfPage.editField[3].data;
				servoProfile()->servoConf[index].min = structureOfPage.editField[6].data;
				servoProfile()->servoConf[index].max = structureOfPage.editField[7].data;
				servoProfile()->servoConf[index].middle = structureOfPage.editField[9].data;
				servoProfile()->servoConf[index].rate = (structureOfPage.editField[10].data ? 1 : -1) * structureOfPage.editField[11].data;
				servoProfile()->servoConf[index].angleAtMin = structureOfPage.editField[12].data;
				servoProfile()->servoConf[index].angleAtMax = structureOfPage.editField[15].data;
				if (structureOfPage.editField[18].data == numberOfElementsInTable(servoForwardChannel) - 1)
				    servoProfile()->servoConf[index].forwardFromChannel = (int8_t)CHANNEL_FORWARDING_DISABLED;
				else
				    servoProfile()->servoConf[index].forwardFromChannel = structureOfPage.editField[18].data;
			}
		break;
	    }
	}
...
static void prepareLinesOfTextForTheSelectedPage(void)
{
    ...
    switch (selectedPage){
        ...
        case FIRMWARE_PAGE : {
			...
			includeEmptyLine(6);
			snprintf(contentOfPage.lineOfText[7],CHARS_PER_LINE, firmwarePage[6], mixerNames[mixerConfig()->mixerMode - 1]);
	    break;
        }
	    case FEATURES_PAGE : {
				...
				if ((selectedPage == RECEIVER_PAGE) && (selectedItem == 18)){
					if (structureOfPage.editField[selectedItem].color == PIXEL_INVERTED)
						...
						if (theRCMappingStringIsValid()){
							parseRcChannels(tempString, rxConfig());
							structureOfPage.editField[selectedItem].color = PIXEL_NORMAL;
							exitEditingProcess();
						}
						else {
