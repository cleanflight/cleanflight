...
uint8_t read8(void)
{
    return currentMspPort->inBuf[currentMspPort->indRX++] & 0xff;
}
...
static void headSerialResponse(uint8_t err, uint8_t responseBodySize)
{
    ...
    serialize8('M');
    serialize8(err ? '!' : '>');
    currentMspPort->checksum = 0;               // start calculating a new checksum
    serialize8(responseBodySize);
    serialize8(currentMspPort->cmdMSP);
}
...
static bool processInCommand(uint8_t cmdMSP)
{
    ...
    const pgRegistry_t *reg = pgMatcher(pgMatcherForMSPSet, (void*)&cmdMSP);
    ...
    if (reg != NULL) {
        ...
        pgLoad(reg, currentMspPort->inBuf + currentMspPort->indRX, currentMspPort->dataSize, getCurrentProfile());
        headSerialReply(0);
        return true;
    }
    ...
    case MSP_SET_CF_SERIAL_CONFIG:
        {
            ...
            uint8_t remainingPortsInPacket = currentMspPort->dataSize / portConfigSize;
        ...
        if (i == 0xFF) {
            ...
            if (i < escCount) {
                ...
                */
                ...
                currentMspPort->cmdMSP = MSP_SET_1WIRE;
            } else {
...
{
    if (currentMspPort->c_state == IDLE) {
        if (c == '$') {
            currentMspPort->c_state = HEADER_START;
        } else {
            return false;
        }
    } else if (currentMspPort->c_state == HEADER_START) {
        currentMspPort->c_state = (c == 'M') ? HEADER_M : IDLE;
    } else if (currentMspPort->c_state == HEADER_M) {
        currentMspPort->c_state = (c == '<') ? HEADER_ARROW : IDLE;
        ...
        if (currentMspPort->checksum == c) {
            currentMspPort->c_state = COMMAND_RECEIVED;
        } else {
            currentMspPort->c_state = IDLE;
        }
    }
