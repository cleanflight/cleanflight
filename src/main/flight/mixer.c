...
static const motorMixer_t mixerQuadXtilt[] = { { 1.0f, -1.0f, 1.0f, -1.0f }, // REAR_R
		{ 1.0f, -1.0f, -1.0f, 1.0f },          // FRONT_R
		{ 1.0f, 1.0f, 1.0f, 1.0f },          // REAR_L
		{ 1.0f, 1.0f, -1.0f, -1.0f },          // FRONT_L
		{ 1.0f, 1.0f, -1.0f, -1.0f },          // TILT
		};
...
static const motorMixer_t mixerOctoX8tilt[] = { { 1.0f, -1.0f, 1.0f, -1.0f }, // REAR_R
		{ 1.0f, -1.0f, -1.0f, 1.0f },          // FRONT_R
		{ 1.0f, 1.0f, 1.0f, 1.0f },          // REAR_L
		{ 1.0f, 1.0f, -1.0f, -1.0f },          // FRONT_L
		{ 1.0f, -1.0f, 1.0f, 1.0f },          // UNDER_REAR_R
...
// motors, servos, motor mixer
		...
		{ 4, 1, mixerQuadXtilt },      // MIXER_QUADX_TILT
		{ 4, 1, mixerQuadXtilt },      // MIXER_QUADX_TILT_COS
		{ 4, 1, mixerQuadXtilt },      // MIXER_QUADX_TILT_THRUST
		{ 4, 1, mixerQuadXtilt },      // MIXER_QUADX_TILT_PITCH
		{ 4, 1, mixerQuadXtilt },      // MIXER_QUADX_TILT_ALL
...
STATIC_UNIT_TESTED void forwardAuxChannelsToServos(void) {
	...
	int8_t servoOffset;
	for (servoOffset = 0; servoOffset < AUX_FORWARD_CHANNEL_TO_SERVO_COUNT;
			servoOffset++) {
...
static void airplaneMixer(void) {
	...
	if (!ARMING_FLAG(ARMED))
		...
		servo[7] = constrain(rcCommand[THROTTLE],
...
void mixTable(void) {
	...
	if (motorCount > 3) {
		...
		axisPID[YAW] = constrain(axisPID[YAW], -100 - ABS(rcCommand[YAW]),
				+100 + ABS(rcCommand[YAW]));
	}
	...
	if (motorCount > 1) {
		for (i = 0; i < motorCount; i++) {
			motor[i] = rcCommand[THROTTLE] * currentMixer[i].throttle
					...
					+ -mixerConfig->yaw_direction * axisPID[YAW]
							* currentMixer[i].yaw;
		}
	}
	...
	switch (currentMixerMode) {
	case MIXER_BI:
		servo[4] = (servoDirection(4, 2) * axisPID[YAW])
				+ (servoDirection(4, 1) * axisPID[PITCH])
				+ determineServoMiddleOrForwardFromChannel(4);     // LEFT
		servo[5] = (servoDirection(5, 2) * axisPID[YAW])
				+ (servoDirection(5, 1) * axisPID[PITCH])
				+ determineServoMiddleOrForwardFromChannel(5);     // RIGHT
		break;

	case MIXER_TRI:
		servo[5] = (servoDirection(5, 1) * axisPID[YAW] * yawDirection3D)
				+ determineServoMiddleOrForwardFromChannel(5); // REAR
		break;

	case MIXER_GIMBAL:
		servo[GIMBAL_SERVO_PITCH] =
				(((int32_t) servoConf[GIMBAL_SERVO_PITCH].rate
						* inclination.values.pitchDeciDegrees) / 50)
						+ determineServoMiddleOrForwardFromChannel(
						GIMBAL_SERVO_PITCH);
		servo[GIMBAL_SERVO_ROLL] = (((int32_t) servoConf[GIMBAL_SERVO_ROLL].rate
	...
	case MIXER_FLYING_WING:
		...
		if (FLIGHT_MODE(PASSTHRU_MODE)) {
			...
			servo[3] = (servoDirection(3, 1) * rcCommand[PITCH])
					+ (servoDirection(3, 2) * rcCommand[ROLL]);
			servo[4] = (servoDirection(4, 1) * rcCommand[PITCH])
					+ (servoDirection(4, 2) * rcCommand[ROLL]);
		} else {
			// use sensors to correct (gyro only or gyro + acc)
			servo[3] = (servoDirection(3, 1) * axisPID[PITCH])
					+ (servoDirection(3, 2) * axisPID[ROLL]);
			servo[4] = (servoDirection(4, 1) * axisPID[PITCH])
					+ (servoDirection(4, 2) * axisPID[ROLL]);
		}
	...
	case MIXER_SINGLECOPTER:
		for (i = 3; i < 7; i++) {
			servo[i] = (axisPID[YAW] * servoDirection(i, 2))
					+ (axisPID[(6 - i) >> 1] * servoDirection(i, 1)); // mix and setup direction
			servo[i] += determineServoMiddleOrForwardFromChannel(i);
		}
	...
	if (feature(FEATURE_SERVO_TILT)) {
		...
		if (IS_RC_MODE_ACTIVE(BOXCAMSTAB)) {
			if (gimbalConfig->gimbal_flags & GIMBAL_MIXTILT) {
				servo[0] -= (-(int32_t) servoConf[0].rate)
						* inclination.values.pitchDeciDegrees / 50
						- (int32_t) servoConf[1].rate
								* inclination.values.rollDeciDegrees / 50;
				servo[1] += (-(int32_t) servoConf[0].rate)
						* inclination.values.pitchDeciDegrees / 50
						+ (int32_t) servoConf[1].rate
								* inclination.values.rollDeciDegrees / 50;
			} else {
				servo[0] += (int32_t) servoConf[0].rate
						* inclination.values.pitchDeciDegrees / 50;
				servo[1] += (int32_t) servoConf[1].rate
						* inclination.values.rollDeciDegrees / 50;
			}
		}
	}
	...
	if (ARMING_FLAG(ARMED)) {
		...
		for (i = 0; i < motorCount; i++) {
			...
			if (feature(FEATURE_3D)) {
				...
				motor[i] = constrain(motor[i], escAndServoConfig->minthrottle,
						escAndServoConfig->maxthrottle);
