...
static float lastServoOutputTiltAngleMode = 0;
#endif
...
static const motorMixer_t mixerOctoFlatP[] = { { 1.0f, 0.707107f, -0.707107f,
		1.0f },    // FRONT_L
...
static const servoMixer_t servoMixerAirplane[] = { { SERVO_FLAPPERON_1,
		INPUT_STABILIZED_ROLL, 100, 0, 0, 100, 0 }, { SERVO_FLAPPERON_2,
		INPUT_STABILIZED_ROLL, 100, 0, 0, 100, 0 }, { SERVO_RUDDER,
		INPUT_STABILIZED_YAW, 100, 0, 0, 100, 0 }, { SERVO_ELEVATOR,
		INPUT_STABILIZED_PITCH, 100, 0, 0, 100, 0 }, { SERVO_THROTTLE,
		INPUT_STABILIZED_THROTTLE, 100, 0, 0, 100, 0 }, };

static const servoMixer_t servoMixerFlyingWing[] = { { SERVO_FLAPPERON_1,
		INPUT_STABILIZED_ROLL, 100, 0, 0, 100, 0 }, { SERVO_FLAPPERON_1,
		INPUT_STABILIZED_PITCH, 100, 0, 0, 100, 0 }, { SERVO_FLAPPERON_2,
		INPUT_STABILIZED_ROLL, -100, 0, 0, 100, 0 }, { SERVO_FLAPPERON_2,
		INPUT_STABILIZED_PITCH, 100, 0, 0, 100, 0 }, { SERVO_THROTTLE,
		INPUT_STABILIZED_THROTTLE, 100, 0, 0, 100, 0 }, };

static const servoMixer_t servoMixerBI[] = { { SERVO_BICOPTER_LEFT,
		INPUT_STABILIZED_YAW, 100, 0, 0, 100, 0 }, { SERVO_BICOPTER_LEFT,
		INPUT_STABILIZED_PITCH, 100, 0, 0, 100, 0 }, { SERVO_BICOPTER_RIGHT,
		INPUT_STABILIZED_YAW, 100, 0, 0, 100, 0 }, { SERVO_BICOPTER_RIGHT,
		INPUT_STABILIZED_PITCH, 100, 0, 0, 100, 0 }, };

static const servoMixer_t servoMixerTri[] = { { SERVO_RUDDER,
		INPUT_STABILIZED_YAW, 100, 0, 0, 100, 0 }, };

static const servoMixer_t servoMixerDual[] = { { SERVO_DUALCOPTER_LEFT,
		INPUT_STABILIZED_PITCH, 100, 0, 0, 100, 0 }, { SERVO_DUALCOPTER_RIGHT,
		INPUT_STABILIZED_ROLL, 100, 0, 0, 100, 0 }, };

static const servoMixer_t servoMixerSingle[] = { { SERVO_SINGLECOPTER_1,
		INPUT_STABILIZED_YAW, 100, 0, 0, 100, 0 }, { SERVO_SINGLECOPTER_1,
		INPUT_STABILIZED_PITCH, 100, 0, 0, 100, 0 }, { SERVO_SINGLECOPTER_2,
		INPUT_STABILIZED_YAW, 100, 0, 0, 100, 0 }, { SERVO_SINGLECOPTER_2,
		INPUT_STABILIZED_PITCH, 100, 0, 0, 100, 0 }, { SERVO_SINGLECOPTER_3,
		INPUT_STABILIZED_YAW, 100, 0, 0, 100, 0 }, { SERVO_SINGLECOPTER_3,
		INPUT_STABILIZED_ROLL, 100, 0, 0, 100, 0 }, { SERVO_SINGLECOPTER_4,
		INPUT_STABILIZED_YAW, 100, 0, 0, 100, 0 }, { SERVO_SINGLECOPTER_4,
		INPUT_STABILIZED_ROLL, 100, 0, 0, 100, 0 }, };

static const servoMixer_t servoMixerGimbal[] = { { SERVO_GIMBAL_PITCH,
		INPUT_GIMBAL_PITCH, 125, 0, 0, 100, 0 }, { SERVO_GIMBAL_ROLL,
		INPUT_GIMBAL_ROLL, 125, 0, 0, 100, 0 }, };
...
void mixerUseConfigs(
		...
		mixerConfig_t *mixerConfigToUse, airplaneConfig_t *airplaneConfigToUse,
...
int16_t determineServoMiddleOrForwardFromChannel(servoIndex_e servoIndex) {
	...
	if (channelToForwardFrom != CHANNEL_FORWARDING_DISABLED
			&& channelToForwardFrom < rxRuntimeConfig.channelCount) {
...
void mixerInit(mixerMode_e mixerMode, motorMixer_t *initialCustomMotorMixers,
		servoMixer_t *initialCustomServoMixers) {
...
void mixerUsePWMOutputConfiguration(
...
void servoMixerLoadMix(int index, servoMixer_t *customServoMixers) {
	...
	// clear existing
	for (i = 0; i < MAX_SERVO_RULES; i++)
		customServoMixers[i].targetChannel = customServoMixers[i].inputSource =
				customServoMixers[i].rate = customServoMixers[i].box = 0;
...
STATIC_UNIT_TESTED void forwardAuxChannelsToServos(uint8_t firstServoIndex) {
	...
	uint8_t servoOffset;
	for (servoOffset = 0;
			servoOffset < MAX_AUX_CHANNEL_COUNT
					&& channelOffset < MAX_SUPPORTED_RC_CHANNEL_COUNT;
			servoOffset++) {
...
void writeServos(void) {
	...
	case MIXER_SINGLECOPTER:
		for (int i = SERVO_SINGLECOPTER_INDEX_MIN;
				i <= SERVO_SINGLECOPTER_INDEX_MAX; i++) {
			pwmWriteServo(servoIndex++, servo[i]);
		}
		break;
...
void stopMotors(void) {
	writeAllMotors(
			feature(FEATURE_3D) ?
					flight3DConfig->neutral3d : escAndServoConfig->mincommand);
...
STATIC_UNIT_TESTED void servoMixer(void) {
	...
	input[INPUT_GIMBAL_PITCH] = scaleRange(inclination.values.pitchDeciDegrees,
			-1800, 1800, -500, +500);
	input[INPUT_GIMBAL_ROLL] = scaleRange(inclination.values.rollDeciDegrees,
			-1800, 1800, -500, +500);
	...
	for (i = 0; i < servoRuleCount; i++) {
		...
		if (currentServoMixer[i].box
				== 0|| IS_RC_MODE_ACTIVE(BOXSERVO1 + currentServoMixer[i].box - 1)) {
			uint8_t target = currentServoMixer[i].targetChannel;
			uint8_t from = currentServoMixer[i].inputSource;
			uint16_t servo_width = servoConf[target].max
					- servoConf[target].min;
			int16_t min = currentServoMixer[i].min * servo_width / 100
					- servo_width / 2;
			int16_t max = currentServoMixer[i].max * servo_width / 100
					- servo_width / 2;
			...
			if (currentServoMixer[i].speed == 0)
				...
				if (currentOutput[i] < input[from])
					currentOutput[i] = constrain(
							currentOutput[i] + currentServoMixer[i].speed,
							currentOutput[i], input[from]);
				else if (currentOutput[i] > input[from])
					currentOutput[i] = constrain(
							currentOutput[i] - currentServoMixer[i].speed,
							input[from], currentOutput[i]);
			}

			servo[target] += servoDirection(target, from)
					* constrain(
							((int32_t) currentOutput[i]
									* currentServoMixer[i].rate) / 100, min,
							max);
		} else {
			currentOutput[i] = 0;
		}
...
void radiantWriteServo(uint8_t index, float angle) {
	...
	if (angle > 0) {
		angle = scaleRangef(angle, 0, +M_PIf / 2, servoConf[index].middle,
				servoConf[index].max);
	} else {
		angle = scaleRangef(angle, 0, -M_PIf / 2, servoConf[index].middle,
				servoConf[index].min);
	...
	//just to be sure
	uint16_t outputPwm = constrain(angle, servoConf[index].min,
			servoConf[index].max);
...
uint8_t hasTiltingMotor() {
	return currentMixerMode == MIXER_QUADX_TILT
			|| currentMixerMode == MIXER_OCTOX_TILT;
}
...
float requestedTiltServoAngle() {
	...
	requestedAngle = constrainf(requestedAngle, degreesToRadians(servoConf[SERVO_TILT_ARM].angleAtMin), degreesToRadians(servoConf[SERVO_TILT_ARM].angleAtMax));
...
void mixTilting(void) {
	...
	if (hasTiltingMotor()
			&& (tiltArmConfig->flagEnabled & TILT_ARM_ENABLE_THRUST)) {
		...
		if (pitchToCompensate > 0 && angleTilt + bodyPitch < M_PIf / 2) { //if there is something to compensate, and only from 0 to 90, otherwise it will push you into the ground
			uint16_t liftOffTrust = ((rxConfig->maxcheck - rxConfig->mincheck)
					* tiltArmConfig->thrustLiftoff) / 100; //force this order so we don't need float!
			uint16_t liftOffLimit = ((rxConfig->maxcheck - rxConfig->mincheck)
					* 80) / 100; //we will artificially limit the trust compensation to 80%
...
void mixTable(void) {
	...
	if (motorCount
			>= 4&& mixerConfig->yaw_jump_prevention_limit < YAW_JUMP_PREVENTION_LIMIT_HIGH) {
		// prevent "yaw jump" during yaw correction
		axisPID[YAW] = constrain(axisPID[YAW],
				-mixerConfig->yaw_jump_prevention_limit - ABS(rcCommand[YAW]),
				mixerConfig->yaw_jump_prevention_limit + ABS(rcCommand[YAW]));
	}
	...
	for (i = 0; i < motorCount; i++) {
		motor[i] = rcCommand[THROTTLE] * currentMixer[i].throttle
				...
				+ -mixerConfig->yaw_motor_direction * axisPID[YAW]
						* currentMixer[i].yaw;
	}
	...
	if (feature(FEATURE_SERVO_TILT)) {
		...
		servo[SERVO_GIMBAL_PITCH] = determineServoMiddleOrForwardFromChannel(
				SERVO_GIMBAL_PITCH);
		servo[SERVO_GIMBAL_ROLL] = determineServoMiddleOrForwardFromChannel(
				SERVO_GIMBAL_ROLL);

		if (IS_RC_MODE_ACTIVE(BOXCAMSTAB)) {
			if (gimbalConfig->mode == GIMBAL_MODE_MIXTILT) {
				servo[SERVO_GIMBAL_PITCH] -=
						(-(int32_t) servoConf[SERVO_GIMBAL_PITCH].rate)
								* inclination.values.pitchDeciDegrees / 50
								- (int32_t) servoConf[SERVO_GIMBAL_ROLL].rate
										* inclination.values.rollDeciDegrees
										/ 50;
				servo[SERVO_GIMBAL_ROLL] +=
						(-(int32_t) servoConf[SERVO_GIMBAL_PITCH].rate)
								* inclination.values.pitchDeciDegrees / 50
								+ (int32_t) servoConf[SERVO_GIMBAL_ROLL].rate
										* inclination.values.rollDeciDegrees
...
void filterServos(void) {
	...
	if (mixerConfig->servo_lowpass_enable) {
		for (servoIdx = 0; servoIdx < MAX_SUPPORTED_SERVOS; servoIdx++) {
			servo[servoIdx] = (int16_t) lowpassFixed(&lowpassFilters[servoIdx],
					servo[servoIdx], mixerConfig->servo_lowpass_freq);

			// Sanity check
			servo[servoIdx] = constrain(servo[servoIdx],
					servoConf[servoIdx].min, servoConf[servoIdx].max);
		}
	}
#if defined(MIXER_DEBUG)
