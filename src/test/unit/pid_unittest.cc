...
TEST(PIDUnittest, TestLuxFloat)
{
    ...
    EXPECT_FLOAT_EQ(pidProfile.P_f[PITCH] * rateErrorRoll, axisPID_P[FD_PITCH]);
    EXPECT_FLOAT_EQ(pidProfile.I_f[PITCH] * rateErrorRoll * dT * 10.0, axisPID_I[FD_PITCH]);
    ...
    EXPECT_FLOAT_EQ(pidProfile.D_f[ROLL] * rateErrorRoll * 10.0, axisPID_D[FD_ROLL]); // D values are multiplied by 10
    EXPECT_FLOAT_EQ(pidProfile.P_f[PITCH] * rateErrorRoll, axisPID_P[FD_PITCH]);
    EXPECT_FLOAT_EQ(3 * pidProfile.I_f[PITCH] * rateErrorRoll * dT * 10.0, axisPID_I[FD_PITCH]);
    EXPECT_FLOAT_EQ(pidProfile.D_f[PITCH] * rateErrorRoll * 10.0, axisPID_D[FD_PITCH]);
